...
export async function GET(
  ...
  // 然后应用认证中间件
  return authMiddleware.admin(async (req: NextRequest, context: any) => {
    try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        url: audio.url, coverImage: audio.coverImage,
        duration: audio.duration,
        filesize: audio.filesize,
        subject: audio.subject,
        speaker: audio.speaker, uploadDate: audio.uploadDate,
        status: audio.status
      }
    });
  ...
  return authMiddleware.admin(async (req: NextRequest, context: any) => {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        url: updatedAudio.url, coverImage: updatedAudio.coverImage,
        duration: updatedAudio.duration,
        filesize: updatedAudio.filesize,
        subject: updatedAudio.subject,
        speaker: updatedAudio.speaker, uploadDate: updatedAudio.uploadDate,
...
export async function DELETE(
  ...
  return authMiddleware.admin(async (req: NextRequest, context: any) => {
    ...
    // 检查音频是否存在
    const existingAudio = await db.query(
      'SELECT id, title, filename FROM audios WHERE id = $1',
      [audioId]
    );
    ...
    // 删除音频记录
    await db.query('DELETE FROM audios WHERE id = $1', [audioId]);
