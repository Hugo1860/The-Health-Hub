...
export const GET = authMiddleware.user(
  async (request: NextRequest, context) => {
    try {
      ...
      const history = historyQuery.all(user.id, limit, offset) as Array<{
        id: number;
        title: string;
        subject: string;
        "uploadDate": string;
...
export const POST = authMiddleware.userWithRateLimit(
  async (request: NextRequest, context) => {
    try {
      ...
      insertQuery.run(user.id, audioId, position, duration);
  ...
  async (request: NextRequest, context) => {
    try {
      ...
      if (audioId) {
        ...
        result = deleteQuery.run(user.id, audioId);
      } else {
        // 清除所有播放历史
        deleteQuery = db.prepare('DELETE FROM play_history WHERE userId = ?');
        result = deleteQuery.run(user.id);
      }
