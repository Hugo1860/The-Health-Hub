...
export async function GET(request: NextRequest) {
  try {
    ...
    let dateFilter = '';
    if (period === 'week') {
      dateFilter = "AND a.upload_date >= NOW() - INTERVAL '7 days'";
    } else if (period === 'month') {
      dateFilter = "AND a.upload_date >= NOW() - INTERVAL '30 days'";
    }
    ...
    const query = `
      SELECT 
        ...
        ROW_NUMBER() OVER (ORDER BY COALESCE(a.play_count, 0) DESC, a.upload_date DESC) as rank
      FROM audios a
    ...
    const rankings = audioResult.rows.map(row => ({
      id: row.id,
      title: row.title,
