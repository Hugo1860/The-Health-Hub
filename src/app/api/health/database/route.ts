...
import { withPostgreSQLErrorHandling } from '@/lib/postgresql-api-middleware';
...
export const POST = apiMiddleware.public(async (req, context) => {
  ...
  try {
    ...
    const responseData: DatabaseReconnectResponse = {
      message: connected ? 'PostgreSQL database reconnected successfully' : 'PostgreSQL database reconnection failed',
      reconnection: {
    ...
    // 数据库重连失败
    throw ApiErrors.DATABASE_ERROR(
      'PostgreSQL database reconnection failed',
      { 
        originalError: error instanceof Error ? error.message : 'Unknown error',
        responseTime: Date.now() - startTime
      }
