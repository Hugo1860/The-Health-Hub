...
export async function GET(request: NextRequest) {
  try {
    ...
    // 构建查询条件
    let statusCondition = "c.status = 'approved'";
    if (includePending) {
      statusCondition = "c.status IN ('approved', 'pending')";
    }
    ...
    const query = `
      ...
      WHERE c.audio_id = $1 AND ${statusCondition}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertQuery = `
      ...
      VALUES (gen_random_uuid()::text, $1, $2, $3, 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      RETURNING *
    `;
